...
// Adapted from 21st.dev's AnimatedThemeToggle:
// - motion/react instead of framer-motion (already a dependency)
// - plain styled <button> instead of the shadcn Button/cn/cva stack —
//   one usage doesn't justify three new packages
// - actually drives the theme: toggles .dark on <html> and persists to
//   localStorage (read at boot by the inline script in layout.tsx)
...
export function AnimatedThemeToggle({ className = "" }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={`inline-flex h-9 w-9 items-center justify-center border border-ink/15 text-ink hover:border-ember hover:text-ember transition-colors ${className}`}
    >
